...
const fs = require('fs');
const pool = require('./db'); // db עם Pool כמו שהצעתי
...
app.get('/api/areas', async (req, res) => {
  try {
    const [results] = await pool.query('SELECT * FROM areas');
    res.json(results);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.get('/api/areas/name/:name', async (req, res) => {
  try {
    ...
    const [results] = await pool.query('SELECT * FROM areas WHERE name = ?', [areaName]);
...
app.put('/api/areas/name/:name', upload.single('path'), async (req, res) => {
  try {
    ...
    if (filePath) {
      ...
      const [oldFile] = await pool.query('SELECT path FROM areas WHERE name = ?', [oldName]);
    ...
    values.push(oldName);
    await pool.query(`UPDATE areas SET ${fields.join(', ')} WHERE name = ?`, values);
...
app.delete('/api/areas/name/:name', async (req, res) => {
  try {
    const areaName = decodeURIComponent(req.params.name);
    const [area] = await pool.query('SELECT path, id FROM areas WHERE name = ?', [areaName]);
    if (!area[0]) return res.status(404).json({ error: 'אזור לא נמצא' });

    if (area[0].path) fs.unlink(path.join(__dirname, 'uploads', area[0].path), () => {});

    await pool.query('DELETE FROM shades WHERE Area = ?', [area[0].id]);
    await pool.query('DELETE FROM areas WHERE id = ?', [area[0].id]);
...
app.post('/api/shades', async (req, res) => {
  try {
    ...
    const [area] = await pool.query('SELECT id FROM areas WHERE name = ?', [Area]);
...
app.put('/api/shades/:id', async (req, res) => {
  try {
    ...
    await pool.query(`UPDATE shades SET ${fields.join(', ')} WHERE id = ?`, values);
...
app.delete('/api/shades/:id', async (req, res) => {
  try {
    await pool.query('DELETE FROM shades WHERE id = ?', [req.params.id]);
